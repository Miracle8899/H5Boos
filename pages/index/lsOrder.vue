<template>
	<view class="supplier">
		...
		<pop ref="pop" direction="below" :is_close="false"   :is_mask="true" :width="100">
			...
			<button type="default" @click="choseEd('1')">正常</button>
			<button type="default" @click="choseEd('2')">单品退货</button>
			<button type="default" @click="choseEd('3')">整单退货</button>
			<button type="default" @click="choseEd('4')">退货订单</button>
			<button type="default" @click="choseEd('5')">作废</button>
			<button type="default" @click="choseEd('6')">错误订单</button>
		...
		<view class="supplier-box" v-for="(item, index) in list" :key="index" @click="goDetail(item)" >
			<uni-card :title="item.store"   thumbnail="../../static/images/face.png">
				<view class="supplier-box-cell">
					...
					<text>订单类型 : {{typeChange(item.type)}}</text>
					<text>收银员 : {{item.salesMan || ''}}</text>
		...
		methods: {
			...
			async getPage (val) {
				...
				try {
					...
					let res =  await this.$api.getRetailInquiry({
						current: this.page,
						model: {
							storeCode: store,
							displaySize: 5,
